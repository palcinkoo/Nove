...
tap_by_text() {
  ...
  if [ -z "$bounds" ]; then
    echo "::error::Tlacidlo '$text' sa nenaslo v UI"
    exit 1
  fi
  local x1 y1 x2 y2
...
adb shell uiautomator dump /sdcard/ui.xml >/dev/null 2>&1 || true
if adb shell cat /sdcard/ui.xml 2>/dev/null | grep -q "Párovanie zariadenia"; then
  echo "OK: Dokoncovacia obrazovka s parovacim kodom sa zobrazila"
else
  echo "::error::Dokoncovacia obrazovka sa nezobrazila - wizard nepresiel az po parovanie"
  exit 1
fi
